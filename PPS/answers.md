...
Stop.
```md Pseudocode for Factorial of a Number:
...
Flow Chart to Check if a Number is Prime:
Start (Oval)

Input number n (Rectangle)

Initialize isPrime = 1 and i = 2 (Rectangle)

Diamond: i <= n/2?
Yes: Diamond: n % i == 0?
Yes: Set isPrime = 0, break (Rectangle)

No: Increment i (Rectangle), loop back to i <= n/2
No: Proceed
Diamond: isPrime == 1?
Yes: Print "Prime" (Rectangle)

No: Print "Not Prime" (Rectangle)
...
A function in C is a block of code designed to perform a specific task, enhancing code reusability and modularity. It is declared with a return type, name, and parameters, executed when called. There are two main types: user-defined functions, created by the programmer to meet specific needs, and built-in functions, provided by the C library (e.g., printf, scanf from <stdio.h>).
...
strlen from <string.h> is a built-in function that returns the length of a string.
Call by Value vs. Call by Reference:
Call by Value: The function receives a copy of the argument’s value. Changes inside the function don’t affect the original variable.
Example:
c
...
Here, a remains unchanged as only a copy is passed.
Call by Reference: The function receives the memory address, allowing changes to the original variable using pointers.
Example:
c
...
Here, a changes because the address is passed, altering the original value. The key difference lies in memory manipulation, with call by reference being more efficient for large data.
